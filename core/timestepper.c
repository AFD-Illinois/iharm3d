...
void grim_timestep(struct GridGeom *G, struct FluidState *Si, 
  struct FluidState *Ss, struct FluidState *Sf, struct FluiFlux *F, double dt)
{
  ...
  static int firstc = 1;
  if (firstc) {
    S_eps = calloc(1, sizeof(FluidState));
  ...
  ZLOOP{
    for (int col = 0; col < NVAR; col++) {
      ...
      if (abs(Sf->P[col][k][j][i]) < 0.5 * EPS) {
        is_small = 1;
      }

      // Compute P_eps
      S_eps->P[col][k][j][i] = Sf->P[col][k][j][i] + EPS*Sf->P[col][k][j][i]*(1 - is_small) + EPS*is_small;
...
#pragma omp parallel for collapse(3)
  PLOOP ZLOOP {
    residual[ip][k][j][i] = (Sf->U[ip][k][j][i] - Si->[ip][k][j][i]) / dt
