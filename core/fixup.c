...
#pragma omp parallel for simd collapse(2) reduction(+:n_geom_rho) reduction(+:n_geom_u) \
  ...
  int n_geom_rho = mpi_reduce_int(n_geom_rho);
  int n_geom_u = mpi_reduce_int(n_geom_u);
  int n_b_rho = mpi_reduce_int(n_b_rho);
  int n_b_u = mpi_reduce_int(n_b_u);
  int n_temp = mpi_reduce_int(n_temp);
  int n_gamma = mpi_reduce_int(n_gamma);
  int n_ktot = mpi_reduce_int(n_ktot);
