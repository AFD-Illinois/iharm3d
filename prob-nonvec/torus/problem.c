...
void init_prob()
{
  ...
  ISLOOP(5, N1-1) {
    ...
    bl_coord(X, &r, &th);
    if (r < rin) {
      double B2net =  (A[i][j] + A[i][j+1] - A[i+1][j] - A[i+1][j+1])/(2.*dx[1]*ggeom[i][j][CENT].g);
      Phi += ggeom[i][j][CENT].g*2.*M_PI*dx[1]/r*fabs(B2net);
    }
  }
  if (global_start[1] == 0) {
    JSLOOP(0, N2/2-1) {
      ...
      Phi += ggeom[i][j][CENT].g*dx[2]*2.*M_PI*fabs(B1net);
    }
  }
  Phi = mpi_reduce(Phi);
