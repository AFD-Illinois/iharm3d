...
if __name__=='__main__':
  ...
  NVAR = 10
  VARS = ['rho', 'u', 'u1', 'u2', 'u3', 'q', 'deltaP', 'B1', 'B2', 'B3']
  RES = [32,64,128,256]
  ...
  dvar_cos[5] = 0.5233486841539436
  dvar_cos[6] = 0.2909106062057657
  dvar_cos[7] = -0.05973794979640743
  dvar_cos[8] = 0.02986897489820372
  dvar_sin = np.zeros(NVAR)
  dvar_sin[0] = 0.1792647678001878
  dvar_sin[2] = -0.011862022608466367
  dvar_sin[3] = 0.034828080823603294
  dvar_sin[5] = -0.04767672501939603
  dvar_sin[6] = -0.02159452055336572
  dvar_sin[7] = 0.03351707506150924
  dvar_sin[8] = -0.016758537530754618
  ...
  for r in range(len(RES)):
    ...
    t = dfile['t'][()]
    ho_terms = dfile['/header/imex/emhd/higher_order_terms_conduction'][()]
    ...
    dump['q'] = dfile['prims'][Ellipsis,5][()]
    dump['deltaP'] = dfile['prims'][Ellipsis,6][()]
    dump['B1'] = dfile['prims'][Ellipsis,7][()]
    dump['B2'] = dfile['prims'][Ellipsis,8][()]
    dump['B3'] = dfile['prims'][Ellipsis,9][()]
    if ho_terms:
      dump['conduction_alpha'] = dfile['/header/imex/emhd/conduction_alpha'][()]
      dump['viscosity_alpha']  = dfile['/header/imex/emhd/viscosity_alpha'][()]
      dump['tau'] = dfile['/header/imex/emhd/tau'][()]

      cs2   = (gam * (gam - 1.) * dump['U']) / (dump['RHO'] + (gam * dump['U']))
      Theta = (gam - 1.) * dump['U'] / dump['RHO']
      dump['chi_emhd'] = dump['conduction_alpha'] * cs2 * dump['tau']
      dump['nu_emhd']  = dump['viscosity_alpha'] * cs2 * dump['tau']

      dump['q']      *= np.sqrt(dump['chi_emhd'] * dump['RHO'] * Theta**2 / dump['tau'])
      dump['deltaP'] *= np.sqrt(dump['nu_emhd'] * dump['RHO'] * Theta / dump['tau'])
    ...
    var_numerical[5,Ellipsis] = dump['q']
    var_numerical[6,Ellipsis] = dump['deltaP']
    var_numerical[7,Ellipsis] = dump['B1']
    var_numerical[8,Ellipsis] = dump['B2']
    var_numerical[9,Ellipsis] = dump['B3']
  ...
  ax.set_xlabel('Resolution (N$^2$)')
  ax.set_ylabel('Error (L1 norm)')
  ax.legend()
