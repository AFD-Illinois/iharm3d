...
def overlay_thphi_contours(ax, geom, r):
  ...
  ax.contour(x,y, prep(avg['betagamma'+s]), [1.0], colors='xkcd:gray')
...
os.mkdir("frames_lthmovie"+run_name.replace("/","_"))
...
#   plt.close(fig)
  ...
  bplt.plot_thphi(ax[0,0], geom, np.log10(avg['FE'][iBZ,:,:]), iBZ, label = "FE 2D Slice r="+rstring)
  overlay_thphi_contours(ax[0,0], geom, 100)
  bplt.plot_thphi(ax[0,1], geom, np.log10(avg['FM'][iBZ,:,:]), iBZ, label = "FM 2D Slice r="+rstring)
  overlay_thphi_contours(ax[0,1], geom, 100)
  bplt.plot_thphi(ax[1,0], geom, np.log10(dump['FL'][iBZ,:,:]), iBZ, label = "FL 2D Slice r="+rstring)
  overlay_thphi_contours(ax[1,0], geom, 100)
  bplt.plot_thphi(ax[1,1], geom, np.log10(dump['RHO'][iBZ,:,:]), iBZ, label = "RHO 2D Slice r="+rstring)
  overlay_thphi_contours(ax[1,1], geom, 100)
  
  plt.savefig(os.path.join("frames_"+run_name.replace("/", "_"), "frame_{:04}.png".format(n)))
  plt.close(fig)
