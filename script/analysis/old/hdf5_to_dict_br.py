...
def load_hdr(fname):
  ...
  keys = ['METRIC', 'ELECTRONS', 'RADIATION',
          'N1', 'N2', 'N3',
  ...
  if 'FULL_DUMP' in dfile:
    hdr['FULL_DUMP'] = dfile['FULL_DUMP'][0]
  else:
    hdr['FULL_DUMP'] = True
...
def load_geom(hdr):
  ...
  # Ask Ben whether this works for him
...
def load_diag(path):
  ...
  if len(dfile) > 15:
    diag['step_made']  = dfile[15]
    diag['step_abs']   = dfile[16]
    diag['step_scatt'] = dfile[17]
    diag['step_lost']  = dfile[18]
    diag['step_rec']   = dfile[19]
    diag['step_tot']   = dfile[20]
    diag['step_sent']  = dfile[21]
    diag['step_rcvd']  = dfile[22]
    diag['step_made_all']  = dfile[23]
    diag['step_abs_all']   = dfile[24]
    diag['step_scatt_all'] = dfile[25]
    diag['step_lost_all']  = dfile[26]
    diag['step_rec_all']   = dfile[27]
    diag['step_tot_all']   = dfile[28]
    diag['step_sent_all']  = dfile[29]
    diag['step_rcvd_all']  = dfile[30]
    diag['tune_emiss']  = dfile[31]
    diag['tune_scatt']  = dfile[32]
    diag['erad'] = dfile[33]
    diag['lum'] = dfile[34]
    diag['eff'] = dfile[35]
