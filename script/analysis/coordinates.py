...
def dxdX_to_KS(X, mtype):
    ...
    ks_t = np.einsum("ij...->...ij", dxdX_KS_to(G, X, mtype))
    return np.einsum("...ij->ij...", np.linalg.inv(ks_t))

def dxdX_KS_to(X, mtype):
    ...
    if mtype == Met.MINKOWSKI:
        ...
        dxdX[1, 1] = np.exp(X[1])
        mks_smooth, poly_norm, poly_xt, poly_alpha = G.mks_smooth, G.poly_norm, G.poly_xt, G.poly_alpha
        ...
        dxdX[1,1] = 1./(X[1] - R0)
        dxdX[2, 1] = -((np.pow(2, 1 + MP0) * np.pow(X[1], -1 + MP0) * MP0 * (MY1 - MY2) * np.atan(((-2 * X[2] + np.pi) * np.tan((H0 * np.pi) / 2.)) / np.pi)) / 
                       (H0 * np.pow(np.pow(X[1], MP0) * (1 - 2 * MY1) + np.pow(2, 1 + MP0) * (MY1 - MY2), 2) * np.pi))
        dxdX[2, 2] = ( (-2 * np.pow(X[1], MP0) * np.tan((H0 * np.pi) / 2.)) / 
                       (H0 * (np.pow(X[1], MP0) * (-1 + 2 * MY1) + 
                              np.pow(2, 1 + MP0) * (-MY1 + MY2)) * np.pi**2 * (1 + (np.pow(-2 * X[2] + np.pi, 2) * np.pow(np.tan((H0 * np.pi) / 2.), 2)) / 
                                                                                                             np.pi**2)))
        dxdX[3,3] = 1.
    elif mtype == Met.EKS:
        dxdX[1,1] = 1. / X[1]
